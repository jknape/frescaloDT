...
tfcalc = function(data, freqs, species, sites, times, sampeff) {
  ...
  tfs = data.table(spec_id = jind, time_id = tind, tf = numeric(ntf), tf_se = numeric(ntf),
                   count = integer(ntf), occ_adj = numeric(ntf), occ_est = numeric(ntf), occ_0 = integer(ntf), occ_098 = integer(ntf), iter_tf = integer(ntf), iter_tf_se = integer(ntf),  conv_tfcalc = logical(ntf))

  sampeff = dcast(sampeff, site_id ~ time_id, value.var = "samp_eff", fill = 0)[sites[,list(site_id)], on = "site_id"]
  ...
  for (l in 1:length(jind)) {
    ...
    if (length(iocc[[l]]) < 1) {
      set(tfs, l, c("tf", "tf_se", "count", "occ_adj", "occ_est", "occ_0", "occ_098", "iter_tf", "iter_tf_se", "conv_tfcalc"),
...
tfcalc0 = function(iocc, s_t, f_j, kmax = 500) {
  ...
  list(tf = tf, tf_se = tf1 - tf, count = n_obs, occ_adj = sptot, occ_est = esttot, occ_0 = ic1, occ_098 = ic2, iter_tf1 = k, iter_tf_se = k2, conv_tfcalc = conv)
}

# Note: info is added to 'sites' as side effect.
frescaDT = function(dat, sites, phi_target = .74, irepmax = 500, fmax=0.99999, fmin=1.0E-10, tol=0.0003) {
  f = dat$freq_obs
  sa_id = dat$site_id[1]
  wgt_n2 = sites$wgt_n2[sa_id]
  alpha = 1
  conv = FALSE
  i = 1L
  ifz = which(f == 0)
  f[f> fmax] = fmax
  f[f < fmin] = fmin
  while (i <= irepmax & ! conv) {
    ff = -log(1-f)
    f_rs = 1 - exp(-ff  * alpha)
    tot = sum(f_rs)
    tot2 = sum(f_rs ^ 2)
    phi = tot2 / tot
    an2 = tot^2 / tot2
    spnum = tot
    if (i < 20) {
      alpha = alpha * exp(1.86 * (log(1-phi) - log(1-phi_target)))
    } else {
      alpha = alpha * phi_target / phi
    }
    if(i == 1) {
      phi0=phi
      an21=an2
      spnum0=tot
    }
    if(abs(phi-phi_target) < tol) {
      conv = TRUE
    }
    i = i + 1
  }
  f_rs = 1-exp(alpha*log(1-f))
  set(sites, i = sa_id, j = c("alpha", "phi_obs", "phi_out", "spnum_obs", "spnum_est", "iter_fresca", "conv_fresca"),
      value =            list(alpha,         phi0,       phi,       spnum0,       spnum,             i,          conv)) # Could preallocate, but not necessary?
  # All of below could be done later, depends only on f, spnum and alpha.
  jrank = order(-f + 1:length(f) * 1e-12)
  sd_f_rs = sqrt(f * (1-f) / wgt_n2)
  ffff = f + sd_f_rs
  ffff[1-ffff < 1e-12] = 1 - 1e-12
  fffff = f - sd_f_rs
  ffsd = 1 - exp(alpha * log(1 - ffff))
  fffsd = 1 - exp(alpha * log(1 - (f - sd_f_rs)))
  sdij = 0.5 * (ffsd - fffsd)
  f_rs[ifz] = 0 # Line 333
  list(f_rs, sdij, order(jrank), order(jrank)/spnum)
}


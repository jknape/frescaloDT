/* Modified from the vegan package.
 * Original code: https://github.com/vegandevs/vegan/blob/master/src/vegdist.c
 */
...
static void veg_distance_n(double *x, int *nr, int *nc, int *from, int *neigh, int *nneigh, double *d, int *method) {
  int j; /* can be a long vector exceeding int maximum */
switch(*method) {
case MANHATTAN:
  distfun = veg_manhattan;
  break;
...
for (j=0; j < *nneigh; j++) {
  if (j % 200 == 199)
    R_CheckUserInterrupt();
  d[j] = distfun(x, *nr, *nc, *from, neigh[j]);
}
}


static void veg_distance_p(double *x, int *nr, int *nc, int *pair, int *np, double *d, int *method) {
  int j; /* can be a long vector exceeding int maximum */
switch(*method) {
case MANHATTAN:
  distfun = veg_manhattan;
...
for (j=0; j < *np; j++) {
    if (j % 200 == 199)
      R_CheckUserInterrupt();
    d[j] = distfun(x, *nr, *nc, pair[j] - 1, pair[j + *np] - 1);
  }
}
...
// Modify to only compute subset of distances
SEXP do_vegdist_n(SEXP x, SEXP from, SEXP neigh, SEXP method)
{
  SEXP dist;
  int nr, nc, nneigh, imethod = asInteger(method);
  R_xlen_t ndis;
  ...
  veg_distance_n(REAL(x), &nr, &nc, INTEGER(from), INTEGER(neigh), &nneigh,  REAL(dist), &imethod);
...
// Modify to only compute subset of distances
SEXP do_vegdist_p(SEXP x, SEXP pair, SEXP method)
{
  SEXP dist;
  int nr, nc, np, imethod = asInteger(method);
  R_xlen_t ndis;
  ...
  veg_distance_p(REAL(x), &nr, &nc, INTEGER(pair), &np,  REAL(dist), &imethod);
